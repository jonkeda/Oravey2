...
public class WorldTemplateViewModelTests
{
    ...
    private static WorldTemplateViewModel CreateVm()
    ...
    public void PresetSelection_FillsAllFields()
    {
        ...
        Assert.Equal("data/srtm", vm.SrtmDirectory);
        Assert.Equal(Path.Combine("data", "noordholland.osm.pbf"), vm.OsmFilePath);
        Assert.Equal("noordholland", vm.RegionName);
        Assert.Equal(Path.Combine("content", "noordholland.worldtemplate"), vm.OutputPath);
    ...
    private static void AddExcluded(WorldTemplateViewModel vm)
    {
