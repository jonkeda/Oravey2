...
public sealed class EnemySpawnerFactory : IEntitySpawnerFactory
{
    ...
    public EnemySpawnerFactory(Game game) => _game = game;

    public bool CanHandle(string prefabId) => prefabId.StartsWith("enemy:", StringComparison.OrdinalIgnoreCase);

    public Entity? Spawn(Scene scene, EntitySpawnInfo spawn, int chunkX, int chunkY)
    {
        var enemyId = spawn.PrefabId["enemy:".Length..];
        ...
        var entity = new Entity($"enemy_{enemyId}");
    ...
    private Model CreateModel()
    {
        ...
        model.Materials.Add(_game.CreateMaterial(new Color(0.8f, 0.15f, 0.15f))); // red
        return model;
    }
}
