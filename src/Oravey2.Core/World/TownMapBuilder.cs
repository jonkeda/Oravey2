...
public static class TownMapBuilder
{
    public static TileMapData CreateTownMap()
    {
        ...
        for (int i = 0; i < 32; i++)
        {
            ...
            map.SetTile(i, 31, TileType.Wall);   // bottom — note: design row 0 = tile y=0
        ...
        map.SetTile(30, 17, TileType.Ground);
        map.SetTile(30, 18, TileType.Ground);
    ...
    private static void SetBuildingWalls(TileMapData map, int x1, int y1, int x2, int y2)
    {
        for (int x = x1; x <= x2; x++)
        {
            map.SetTile(x, y1, TileType.Wall); // top edge
            map.SetTile(x, y2, TileType.Wall); // bottom edge
        }
        for (int y = y1; y <= y2; y++)
        {
            map.SetTile(x1, y, TileType.Wall); // left edge
            map.SetTile(x2, y, TileType.Wall); // right edge
        }
    }
}
