...
public partial class WorldTemplateView : ContentPage
{
    private readonly WorldTemplateMapDrawable _drawable = new();
    private PointF _panStart;
    private string _activeTab = "Towns";

    public WorldTemplateView()
    {
        InitializeComponent();
        BindingContext = App.Current?.Handler?.MauiContext?.Services.GetService<WorldTemplateViewModel>();

        MapCanvas.Drawable = _drawable;
        MapCanvas.EndInteraction += OnMapClicked;

        if (BindingContext is WorldTemplateViewModel vm)
        {
            var presetsDir = Path.Combine(AppContext.BaseDirectory, "data", "presets");
            if (!Directory.Exists(presetsDir))
                presetsDir = Path.GetFullPath("data/presets");
            vm.LoadPresetsFromDirectory(presetsDir);
            ...
            {
                if (args.PropertyName == nameof(WorldTemplateViewModel.SelectedPreset) && vm.SelectedPreset is not null)
                {
    ...
    {
        if (BindingContext is not WorldTemplateViewModel vm) return;
    ...
    private async void OnBrowseRegion(object? sender, EventArgs e)
    {
        ...
        RegionPreset? selected = null;
        pickerVm.RegionSelected += preset => selected = preset;

        await Navigation.PushModalAsync(dialog);

        // Wait for dialog to close by checking if we're still on the modal
        var tcs = new TaskCompletionSource();
        dialog.Disappearing += (_, _) => tcs.TrySetResult();
        await tcs.Task;
    ...
    private async void OnBrowseSrtm(object? sender, EventArgs e)
    {
        ...
        if (result?.Folder?.Path is string path && BindingContext is WorldTemplateViewModel vm)
    ...
    private async void OnBrowseOsm(object? sender, EventArgs e)
    {
        ...
        if (result?.FullPath is string path && BindingContext is WorldTemplateViewModel vm)
            vm.OsmFilePath = path;
    }
    ...
    private async void OnBrowseOutput(object? sender, EventArgs e)
    {
        var options = new PickOptions
        {
            ...
            FileTypes = new FilePickerFileType(new Dictionary<DevicePlatform, IEnumerable<string>>
            {
                { DevicePlatform.WinUI, [".worldtemplate"] }
            })
        };
        var result = await FilePicker.Default.PickAsync(options);
        if (result?.FullPath is string path && BindingContext is WorldTemplateViewModel vm)
